...
public class PetDetailActivity extends AppCompatActivity {
    private TextView txtTitle,txtCity,txtQuantity,txtDescription,edAge;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ReturnBtn.setOnClickListener(view -> ReturnPet());
...
//                        Picasso.get().load( dataSnapshot.child( "imageurl" ).getValue().toString() ).into( imageView );
                            ...
                            Glide.with(PetDetailActivity.this).load(Objects.requireNonNull(dataSnapshot.child("imageurl").getValue()).toString()).into(CareTakerImage);
    ...
    private void ReturnPet() {
        ...
        firebaseDatabase.child("Pets").child(petDetail.getMyPet_ID()).updateChildren(map).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
        ...
        firebaseDatabase.child("Pets").orderByKey().limitToLast(1).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                firebaseDatabase.child("Pets").child(petDetail.getMyPet_ID()).updateChildren(map).addOnCompleteListener(task -> {
                    if (task.isSuccessful())
                    {
                        Toast.makeText(PetDetailActivity.this, "Data Updated Successfully", Toast.LENGTH_SHORT).show();
                    }
                    else
                    {
                        Toast.makeText(PetDetailActivity.this, "Unable to Update Data", Toast.LENGTH_SHORT).show();
                    }
    ...
    private void DeleteAd() {
        ...
        firebaseDatabase.child("Ads").child(petDetail.getMyPet_ID()).updateChildren(map).addOnCompleteListener(task -> {
            if (task.isSuccessful())
            {
                Toast.makeText(PetDetailActivity.this, "Data Deleted Successfully", Toast.LENGTH_SHORT).show();
            }
            else
            {
                Toast.makeText(PetDetailActivity.this, "Unable to Delete Data", Toast.LENGTH_SHORT).show();
            }
        });
        progressBar.setVisibility(View.GONE);
